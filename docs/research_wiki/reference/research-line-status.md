...
**Last swept: 2026-07-21 (evening — sequential-axis results signed, 1w overwrite resolved).**
...
| Line | Status | Outcome | Key docs |
|------|--------|---------|----------|
| **Chain-clustering probe** (chaining: mode or coin flip?) | ⛔ **closed (null, SIGNED 2026-07-21)** | `no_chain_clustering` under the hub-guarded dual gate (full-array p=0.022 alone would have over-claimed; single-file p=0.061). Consequence (locked): per-leg feature, no regime model. Re-test = new prereg. | [results](../reviews/btc-fib-chain-clustering-probe-results-20260721.md) |
| **Sequential-feature study** (`chained_origin` → ranking signal?) | ⛔ **closed (null, SIGNED 2026-07-21)** | `no_sequential_feature_signal` (teacher-forced): AP 0.071→0.053, CI [−0.082, +0.045]; univariate signal (17.6 % vs 3.8 %) doesn't convert. **Sequential axis BOUNDED (3 signed results): chaining = byproduct of the zigzag rhythm.** Any chase = new prereg. | [results](../reviews/btc-fib-sequential-feature-results-20260721.md) |
