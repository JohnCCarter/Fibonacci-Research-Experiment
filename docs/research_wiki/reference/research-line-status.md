...
| **Sequential-feature study** (`chained_origin` — does chaining convert to ranking signal?) | ⏳ **pending-input** | **RAN 2026-07-21** (teacher-forced; prereg §9 A1–A7 after two-pass leakage review — self-leg guard, misclick exclusion) → **4h `no_sequential_feature_signal`**: AP 0.0710→0.0532, CI [−0.082, +0.045]. Univariate signal exists (17.6 % vs 3.8 %) but the geometric features already carry it. **Axis now bounded by 3 locked results** → chaining = byproduct of the zigzag rhythm, not a missing driver; lever = contrastive capture. Advisory pending sign-off. | [prereg](../reviews/btc-fib-sequential-feature-prereg-20260721.md), [results](../reviews/btc-fib-sequential-feature-results-20260721.md) |
| **Chain-clustering probe** (is sequential chaining a mode or a per-leg coin flip?) | ⏳ **pending-input** | **RAN 2026-07-21** (autonomous, prereg + pre-run leakage review whose hub-coupling guard A1 changed the outcome) → **4h `no_chain_clustering`**: full-array adjacency p=0.022 but single-file-restricted p=0.061 fails the locked dual gate; Markov CI includes 0. Weak tendency compatible, not demonstrated. **Consequence: sequential conditioning = per-leg feature, no regime model.** Advisory pending owner sign-off. | [prereg](../reviews/btc-fib-chain-clustering-probe-prereg-20260721.md), [results](../reviews/btc-fib-chain-clustering-probe-results-20260721.md) |
...
| **Track-A implicit-negative audit** (2026-07-20 audit item) | ✅ **complete** | Descriptive audit (no verdict): 4h near-miss contamination 0.5 % (ε band fine) but **75 % of implicit negatives coverage-weak** — "never reviewed" ≠ "rejected"; low absolute AP partly a passive-corpus coverage artifact; **contrastive capture (#42) priority raised**. No Stage-2 re-run without fresh prereg. | [audit](../reviews/btc-fib-track-a-implicit-negative-audit-20260721.md) |
