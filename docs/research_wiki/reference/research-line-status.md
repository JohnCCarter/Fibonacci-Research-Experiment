...
| **Cascade-conditioning probe** (does the previous fib predict the next origin? Sequential/U1 hypothesis on existing facit — no new labels) | ⏳ **pending-input** | **RAN 2026-07-20** (after pre-run §9 validity amendments; leakage review: H1a/N1 clean) → **`sequential_origin_signal` on 4h**: H1a 0.256 vs null 0.005, p<0.0005, CI [0.205, 0.298], N=363; ~all EXACT chains (76/93), both directions; 1d+1w context agree, 1M nothing. ~1 in 4 origins = previous endpoint → a **component**, not the selector. **Advisory — waiting on owner sign-off.** | [prereg](../reviews/btc-fib-cascade-conditioning-probe-prereg-20260720.md), [results](../reviews/btc-fib-cascade-conditioning-probe-results-20260720.md) |
