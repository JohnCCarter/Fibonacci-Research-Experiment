...
| [btc-fib-to-genesis-v2-phase1-feature-export-spec-20260615.md](btc-fib-to-genesis-v2-phase1-feature-export-spec-20260615.md) | **Phase 1 feature-export spec** (docs-only) — future causally-safe data contract Fib → Genesis V2. Defines two tables (zone registry + bar feature) with the binding `known_after_ts = max(anchor_b)+buffer` knowability rule and per-row `known_after_ts ≤ timestamp` invariant, 3 baseline table specs (causal swing / placebo / ATR), a thin read-only CSV-first Genesis ingestion contract, 9 causal invariants, a do-not-export list, Phase 2 (dummy-file test) preview, stop/go. Builds nothing; rec: small docs-only Phase 2 next, but needs an explicit go. |
