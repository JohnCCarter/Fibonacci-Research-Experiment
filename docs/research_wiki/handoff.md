...
- **2026-06-18 Fib SELECTION-LEARNING Stage-2 headline BUILT + RUN → POINT ESTIMATE (inference PENDING, no claim yet).** `research/selection_learning.py` (+15 tests): causal truncate-and-whitelist features at `anchor_b+k` (k=3 live set `{magnitude,cleanliness,duration,prominence,structure_alignment}`), re-detected candidate universe, ε-match to human legs, purged split, numpy logistic regression vs §6 magnitude baseline, pooled-AP primary (A5.1). **Only 4h powered** (65 test pos): AP model 0.057 vs base 0.005 (≈11×), AUC≈0.91; AUC≈0.88–0.91 on 1M/1d too. **NOT confirmed** — `lift_pos_powered` is a point-estimate flag, no CI/p-value yet. **Next: inference on the AP-lift (resampled by decision point), then k-sweep/W/gap/Stage-1.** [Results](reviews/btc-fib-selection-learning-results-20260618.md). Artifacts gitignored.
