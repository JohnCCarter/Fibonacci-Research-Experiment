...
def main() -> int:
    ...
    ctx = (
        ...
        "You are the PRIMARY lead agent (architect, planner, reviewer, final verifier).\n"
        "Qwen3-Coder is the implementation specialist.\n"
        "Do NOT let both models freely change scope.\n\n"
        "Your job: inspect wiki/repo, produce a clear implementation handoff, review Qwen output, "
        "final verification. Avoid large code edits here unless user explicitly asks.\n\n"
        "Workflow: GLM plan -> delegate qwen-implementer subagent -> GLM review -> "
        "delegate fixes -> GLM verify.\n"
        "Implement subagent: .cursor/agents/qwen-implementer.md (Qwen is GLM's subagent).\n"
