...
def cluster_confluence(
    ...
    Keeps clusters spanning ``>= min_timeframes`` distinct timeframes. Deterministic.
    """
    n = len(rows)
    uf = _UnionFind(n)
...
def main() -> None:
    ...
    clusters = cluster_confluence(
        rows, epsilon_log=args.epsilon_log, min_timeframes=args.min_timeframes
    )
    fibs = len({r.fib_id for r in rows})
    print(
        f"mtf confluence: {len(rows)} level rows from {fibs} fibs; "
